...
TYPEDEFS and MEMBER CONSTANTS for the transaction_log class:
    ...
    typedef double add_type;
        ...
        However, easier to use than creating/**
 *
*** Data Structures - 9 A.M. **
...
Attempt Number: 2
Purpose: Building out the transaction_log class for a Smart Bank
Purpose for Update: Header fix. The invariant requirement 'to do' was added with yellow highlighting (pun intended).
...
TYPEDEFS and MEMBER CONSTANTS for the transaction_log class:
 ...
 TODO: Reserves will grow at a doubling rate e.g. 1, 2, 4, 8, 16, 32 ... and so on. Is_item unchanged.
    ...
    void reserve(size_type new_capacity)
        ...
        Postcondition: The transaction_log's capacity is changed to the new_capacity
            ...
            is reached.
TODO: Copies an existing rhs object into the lhs. If nullptr, destroy the log and assign log nullptr. Is_item
 becomes what lhs is_item is.

    transaction_log& operator=(const transaction_log &)
        A memory safe copy constructor
        Precondition: N/A
        Postcondition: A copy of the passed log is returned
TODO: Move the cursor (is_item), no matter the state, to the first logged transaction.
...
TODO: is_item (aka cursor) is not changed on lhs object. lhs combines with rhs, if current is valid.
 ...
 CONSTANT MEMBER FUNCTIONS for the sequence class: TODO: is_item is not changed for this section of functions.
...
TODO: Helpful function to avoid duplicate code & dictate control flow, dependant on a valid is_item.
...
TODO: Requires a valid is_item (aka cursor) and cannot function without that state. Returning the default constructor to the type
 (e.g. zero)  is misleading
...
TODO: Requires a valid is_item and cannot function without that state. Returning the default constructor to the type
 (e.g. zero)  is misleading
